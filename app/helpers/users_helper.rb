module UsersHelper
  def render_user_info(user)
    content_tag(:div, class: 'user-info') do
      ...
      render_approval_form(user, 'button margin-top') + render_disapproval_form(user, 'button margin-top')
    end
  end

  private

  def render_approval_form(user, button_class)
    if user.approved == 'false' || user.approved.nil?
      form_for(user, url: update_approval_path(user), method: :put, data: { turbo_stream_target: "user_#{user.id}" }) do |f|
        content_tag(:div) do
          ...
          concat f.submit('Zatwierdź', class: button_class, data: { turbo_stream_frame: 'reload', turbo_confirm: 'Czy na pewno chcesz zatwierdzić ten profil?' })
        end
      end
    end
  end

  def render_disapproval_form(user, button_class)
    if user.approved == 'false' || user.approved.nil?
